...
# ============================================================
# NIFTY PRO QUANT TERMINAL v13.1 (MOTO G85 MOBILE OPTIMIZED)
# ============================================================
...
def main(page: ft.Page):
    ...
    ai_flow_content = ft.Container(
        expand=True, bgcolor="#0A1128", border_radius=12, padding=10,
    ...
    macro_content = ft.Container(
        expand=True, bgcolor="#0A1128", border_radius=12, padding=12,
        border=ft.border.all(1, "#1C2A4A"),
        content=ft.Column([
    ...
    header_row = ft.Row([
        ft.Column([
            ...
            ft.Text("MOTO G85 MOBILE TERMINAL v13.1", size=8, color=ft.colors.CYAN_700, weight=ft.FontWeight.BOLD)
        ], spacing=1),
        time_text
    ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)

    price_text = ft.Text("₹0.00", size=30, weight=ft.FontWeight.BOLD, color=ft.colors.WHITE)
    live_status = ft.Text("Waiting...", size=10, color=ft.colors.WHITE54)
    ...
    chart_container = ft.Container(content=line_chart, height=110, padding=8, bgcolor="#0A1128", border_radius=10, visible=False)

    sup_text = ft.Text("--", size=11, weight=ft.FontWeight.BOLD, color=ft.colors.GREEN_400)
    res_text = ft.Text("--", size=11, weight=ft.FontWeight.BOLD, color=ft.colors.RED_400)
    vix_text = ft.Text("--", size=11, weight=ft.FontWeight.BOLD, color=ft.colors.YELLOW_400)
    atr_text = ft.Text("--", size=11, weight=ft.FontWeight.BOLD, color=ft.colors.PURPLE_300)
    vwap_text = ft.Text("--", size=11, weight=ft.FontWeight.BOLD, color=ft.colors.BLUE_300)
    ...
    data_row = ft.Container(
        ...
        bgcolor="#0A1128", padding=8, border_radius=10, border=ft.border.all(1, "#1C2A4A")
    )

    engine_news_text = ft.Text("Awaiting Macro Scan...", size=9, color=ft.colors.WHITE70)
    engine_tech_text = ft.Text("Awaiting Quant Scan...", size=9, color=ft.colors.WHITE70)
    engine_live_text = ft.Text("Awaiting Price Action...", size=9, color=ft.colors.WHITE70)

    def engine_box(title, icon, icon_color, ref):
        return ft.Container(
            content=ft.Column([
                ft.Row([ft.Icon(icon, size=12, color=icon_color), ft.Text(title, size=9, weight=ft.FontWeight.BOLD, color=icon_color)]),
                ref
            ], spacing=2),
            bgcolor="#0A1128", padding=6, border_radius=8,
            border=ft.border.only(left=ft.border.BorderSide(3, icon_color))
        )

    box_news = engine_box("ENGINE 1: MACRO / SENTIMENT", ft.icons.PUBLIC, ft.colors.ORANGE_400, engine_news_text)
    ...
    final_verdict_text = ft.Text("RUN LIVE SCAN TO GENERATE SETUP", size=11, weight=ft.FontWeight.W_900, color=ft.colors.WHITE)
    entry_text = ft.Text("ENTRY: --", size=10, weight=ft.FontWeight.BOLD, color=ft.colors.WHITE)
    target_text = ft.Text("TARGET: --", size=10, weight=ft.FontWeight.BOLD, color=ft.colors.GREEN_300)
    sl_text = ft.Text("SL: --", size=10, weight=ft.FontWeight.BOLD, color=ft.colors.RED_300)
    reason_text = ft.Text("REASON: Awaiting market scan...", size=9, color=ft.colors.CYAN_200, italic=True)
    ...
    final_box = ft.Container(
        ...
        bgcolor="#070C1E", padding=10, border_radius=10, border=ft.border.all(2, ft.colors.BLUE_700)
    )
    ...
    def run_backtest(e):
        ...
        backtest_result_text.value = "⏳ Fetching Historical Dataset...\nRunning random window simulation..."
        page.update()
        ...
        def worker():
            try:
                ...
                if df is None or df.empty or len(df) < 30:
                    raise Exception("Insufficient data received from server.")
                ...
                backtest_result_text.value = f"⚠️ API Limit Reached.\nError: {ex}"
        ...
        def background_task():
            try:
                ...
                if price > vwap and candle_bull:
                    ...
                    state["latest_reason"] = f"Bullish structure verified. Volume: {volume_msg}."
                elif price < vwap and not candle_bull:
                    signal = "🔴 BEARISH SETUP (PUT)"
                    signal_color = ft.colors.RED_400
                    entry, target, stop = price, price - max(atr * 0.35, 1), price + max(atr * 1.5, 1)
                    state["latest_reason"] = f"Bearish structure verified. Volume: {volume_msg}."
                else:
    ...
    scan_btn = ft.ElevatedButton("LIVE SCAN", icon=ft.icons.RADAR, bgcolor=ft.colors.BLUE_700, color=ft.colors.WHITE, on_click=fetch_all_data, expand=True)
    backtest_btn = ft.ElevatedButton("SCALP TEST", icon=ft.icons.STAR, bgcolor=ft.colors.PURPLE_700, color=ft.colors.WHITE, on_click=run_backtest, expand=True)

    btn_row = ft.Row([scan_btn, backtest_btn], spacing=10)
    auto_scan_switch = ft.Switch(label="Auto Scan (5s)", value=False, active_color=ft.colors.GREEN_400)
    switch_row = ft.Row([auto_scan_switch], alignment=ft.MainAxisAlignment.CENTER)

    # Terminal Container (Tab 1 content)
    ...
    # Mobile Bottom/Top Tabs Navigation for Moto G85
    main_tabs = ft.Tabs(
        selected_index=0,
